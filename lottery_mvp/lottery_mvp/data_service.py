...
from db import log_fetch_result, log_update, select_draws, upsert_draws
    ...
    for lottery_type in LOTTERY_CONFIG:
        try:
            ...
            log_fetch_result(
                ...
                "success",
                False,
                "official source fetched",
                row["issue"],
            )
        except Exception as exc:
            if not ALLOW_DEMO_FALLBACK:
                log_fetch_result(lottery_type, "", "failed", False, str(exc), "")
            ...
            log_fetch_result(lottery_type, "", "fallback", True, str(exc), row["issue"])
        rows.append(row)
    count = upsert_draws(rows)

            ...
            if not cnt:
                conn.execute(
                    ...
                    (code, plan_type, status, expires_at, max_uses, used_count,
                     bound_device, note, created_at, created_by)
...
def create_access_codes(count: int, plan_type: str = "paid", days_valid: int = DEFAULT_ACCESS_CODE_DAYS, note: str = ""):
    created = []
    now = datetime.utcnow()
        ...
        with get_sqlite_conn() as conn:
            for _ in range(count):
                ...
                conn.execute(
                    ...
                    """,
                    (code, plan_type, expires.isoformat(), note, now.isoformat()),
                )
                created.append(code)
    return created
